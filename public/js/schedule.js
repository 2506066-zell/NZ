...
async function loadSchedule() {
  ...
  const today = new Date().getDay() || 7; // JS Sunday is 0, we use 7
  ...
  // Render cards 1-6 (Mon-Sat), or 7 if exists
  for (let d = 1; d <= 7; d++) {
    // Skip empty days unless it's Monday-Friday (show empty placeholder) or it's Today
    ...
    if (!grouped[d].length) {
      ...
      grouped[d].forEach(c => {
        ...
        item.innerHTML = `
          <div class="class-time"><i class="fa-regular fa-clock"></i> ${c.start} - ${c.end}</div>
